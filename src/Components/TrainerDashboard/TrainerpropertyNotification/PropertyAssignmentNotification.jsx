...
const PropertyAssignmentNotification = () => {
  ...
  const handleAcceptClick = (propertyId) => {
    const property = mockPropertyAssignments.find(p => p.id === propertyId);
  ...
  // Actual accept/reject handlers called after confirmation
  const handleConfirmAction = async () => {
    if (!confirmationProperty || !confirmationAction) return;
    ...
    try {
      ...
      if (confirmationAction === 'accept') {
        console.log(`Accepted property assignment for property ID: ${confirmationProperty.id}`);
        setAcceptedProperties(prev => [...prev, confirmationProperty.id]);
      } else if (confirmationAction === 'reject') {
        console.log(`Rejected property assignment for property ID: ${confirmationProperty.id}`);
        setRemovedProperties(prev => [...prev, confirmationProperty.id]);
        // If the rejected property was selected, clear the selection
        if (selectedProperty && selectedProperty.id === confirmationProperty.id) {
    ...
    <div className="assignednotification-container">
      ...
      <div className="assignednotification-content">
                ...
                <div key={property.id}>
                  <div className="assignednotification-table-row">
                    ...
                    {acceptedProperties.includes(property.id) ? (
                      <button className="assignednotification-accepted-btn" disabled>
                        ACCEPTED
                      </button>
                    ) : (
                      <>
                        <button
                          ...
                          onClick={() => handleAcceptClick(property.id)}
                        >
                          Accept
                        </button>
                        <button
          ...
          {selectedProperty && (
            <PropertyCard
              ...
              onAccept={(id) => {
                ...
                setSelectedProperty(null); // auto-close after initiating action
              }}
              onReject={(id) => {
        ...
        <ConfirmationModal
          ...
          propertyName={confirmationProperty?.name}
